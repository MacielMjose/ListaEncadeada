...
void ordena_lista( NODO* *l ){
	...
	// 1- verificar se a lista vazia  ou se só tem um registro
	
	NODO *ant, *proximo;
	INFORMACAO aux; //variavel para realizar a troca
	
	if(*l != NULL && (*l)->prox != NULL){
		
			ant = *l; //anterior recebe o endereço de l, aponta para o L - posiciona ambos no inicio
			proximo = ant->prox;//proximo recebe o endereço de L, aponta pra o L - posiciona ambos no inicio
			
			while(ant->prox != NULL){ //controla o ant, percorre ate que seu proximo seja NULL
				
					while(proximo->prox != NULL) //controla o proximo, percorre ate que seu proximo seja NULL
					{
						if(ant->info.codigo > proximo->info.codigo)	{ //se o codigo de ant é 
							
							aux = ant->info;
							
							ant->info = proximo->info;
							proximo->info = aux;
						
						}
						proximo = proximo->prox; //proximo anda para o proximo de si mesmo
						
					}
					ant = ant->prox; //anterior anda para o proximo de si mesmo		
		}
